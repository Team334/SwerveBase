...
/**
 * Originally by FRC team 254, modified by 6328, and further modified by 167. 
 *
...
public class SwerveSetpointGenerator {
  ...
  private final Rotation2d kPi = new Rotation2d(Math.PI);
  ...
  public SwerveSetpoint generateSetpoint(
    ...
    return generateSetpoint(prevSetpoint, desiredState, Robot.kDefaultPeriod);
  }
  ...
  public SwerveSetpoint generateSetpoint(
    ...
    desiredState = ChassisSpeeds.discretize(desiredState, Robot.kDefaultPeriod);
    ...
    for (int i = 0; i < 4; ++i) {
      ...
      if (desiredModuleState[i].speedMetersPerSecond < 0.0) {
        desired_heading[i] = desired_heading[i].rotateBy(kPi);
      }
    ...
    for (int i = 0; i < 4; ++i) {
      ...
      if (flipHeading(deltaRotation)) {
        retStates[i].angle = retStates[i].angle.rotateBy(kPi);
        retStates[i].speedMetersPerSecond *= -1.0;
      }
